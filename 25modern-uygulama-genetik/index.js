...
ekleButton.addEventListener("click", (e) => {  //! arrow function ile değiştik
        e.preventDefault();
        let sira = ekleneceklerArrayi.length +1;
        let veri = inputDom.value;
        const todo = new Todo(sira,veri)
        ekleneceklerArrayi = [...ekleneceklerArrayi, todo]
        //! ekleneceklerArrayi.push(todo);  yukarıdakini kullandık modern olan spread operatoru
...
class UI { //classların baş harfi buyuk olr
    ...
    static displayTodos(){
        ...
        if (ekleneceklerArrayi.length === 0) {
            ...
            ekleneceklerArrayi.map((item) => { //foreach herdöndüğünde demek. //! map le değiştik forEachi modern js
                result += `                  
                <li>
    ...
    static removeTodo(){
        ulListe.addEventListener("click", (e) => {   //! arrow function ile değiştik
    ...
    static clearTodos(){
        clearButton.addEventListener("click", () => {  //! arrow function ile değiştik
            ekleneceklerArrayi = [];
            Storage.addTodoStorage(ekleneceklerArrayi);
            UI.displayTodos();
...
window.addEventListener("DOMContentLoaded",  () => { //sayfa ilk yüklendiğinde çağrılacaklar  //! arrow function ile değiştik
    UI.removeTodo();
    UI.displayTodos();
    UI.clearTodos();
})
